...
def get_pkgname(apk_path):
    ...
    # sometimes launcher is empty or launcher starts with ".", consistent
...
def explore_actionable_widgets(activity, actionable_widgets, appname):
    ...
    for res_id, widget_type in actionable_widgets.items():
        ...
        if '/.' in return_content:
            current_activity = return_content.split('/.')[1].split(' ')[0]
            record_act = return_content.split('/.')[0].split(' ')[-1] + '.' + current_activity
        else:
            current_activity = return_content.split('.')[-1].split(' ')[0]
            record_act = return_content.split('/')[1].split(' ')[0]
        ...
        if current_activity in activity or "Launcher" in current_activity:
            # the click does not change the UI page
            continue
            ...
            pair[activity] = '->' + widget_type[0] + '->' + record_act
...
def parseManifest(appname):
    ...
    if not os.path.exists(decompile_Path):
        print "cannot find the decompiled app folder: " + appname
        return
...
def exploreAct(apk_path, apk_name, apkresult_folder):
    ...
    Install apk
    '''
    installInfo = device.installAPP(apk_path, apk_name)

    if installInfo == 'Failure':
        return pairs.keys(), installInfo
