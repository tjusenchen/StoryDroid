...
@app.route('/graphComponents')
def graphComponents():
    return render_template('graphComponents.html')
@app.route('/do')
def do():
...
@app.route('/loadingCom', methods=['GET', 'POST'])
def loadingCom():
    return render_template('loadingCom.html', tempL=1)
...
def get_post_dataAct():
    ...
    for cl in os.listdir(os.path.join(componentsPath, jsdata)):
        # print os.path.join(componentsPath, jsdata)
        if os.path.isdir(os.path.join(componentsPath, jsdata, cl)):
            continue
        shutil.copyfile(os.path.join(componentsPath, jsdata, cl), os.path.join(jsPath, 'temp', cl))
        print os.path.join(componentsPath, jsdata, cl)
        print cl

    return jsdata


@app.route('/postoutputCom', methods = ['POST'])
def get_post_dataCom():
    jsdata = request.form['javascript_data']
    print(jsdata)
    componentsPath = (dirNow) + '/static/components'
    jsPath = (dirNow) + '/static/js'
    f = open(os.path.join(jsPath, 'temp', jsdata + 'Utg.js'), "r")
    txtAll = f.read()

    with open(os.path.join(jsPath, 'mainUtg2.js'), 'w') as can:
        can.write(txtAll)
    # jsPath = (dirNow) + '/static/js'
    # newPath = (dirNow) + '/static/actJS'
    #
...
def read_excel():
    excel_path = "/home/zyx/Documents/log1101.xls"
    sheet_num = 0
    resultsList = []
